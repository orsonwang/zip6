...
- [x] 英文地址翻譯（完全匹配時顯示）
- [x] 主題切換（亮色/暗色）
- [x] 版本資訊顯示（內嵌於 HTML，無需 API 呼叫）

### 地址正規化功能

- [x] 阿拉伯數字段名轉中文（1段 → 一段）以正確搜尋資料庫
- [x] 中文數字轉阿拉伯數字（一號 → 1號、十二巷 → 12巷）
- [x] 連續中文數字支援（二二六號 → 226號、一二三號 → 123號）
- [x] 大寫中文數字支援（壹貳參肆伍陸柒捌玖拾佰）
- [x] 全形轉半形（１２３ → 123、Ａ棟 → A棟）
- [x] 台 → 臺 自動轉換（台北市 → 臺北市）
- [x] 樓層格式轉換（一樓之一 → 1樓之1）
...
├── internal/
│   ├── address/
│   │   ├── parser.go                 # 地址解析器（含中文數字轉換）
│   │   ├── parser_test.go            # 解析器測試（186+ 測試案例）
...
- [x] 主題切換正常運作
- [x] 英文地址翻譯（完全匹配時顯示）
- [x] 中文數字地址正規化（一二三號 → 123號、二二六號 → 226號）
- [x] 阿拉伯數字段名轉換（1段 → 一段）
- [x] 版本資訊顯示
- [x] 186+ 測試案例全數通過
- [ ] 可成功打包為單一執行檔
